...
if (isset($_POST['itemID']) AND !empty($_POST['itemID']) AND isset($_POST['itemAction'])) {
    ...
    $print_count = count($_SESSION['kartubuku']??[]);
    ...
    foreach ($_POST['itemID'] as $itemID) {
        ...
        $_SESSION['kartubuku'][$itemID] = $itemID;
    }
    
    echo '<script type="text/javascript">top.$(\'#queueCount\').html(\''.count($_SESSION['kartubuku']).'\');</script>';
...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
    ...
    if (!isset($_SESSION['kartubuku'])) {
        utility::jsToastr('Labels Printing', __('There is no data to print!'), 'error');
        die();
    }

    // biblio data
    $template = file_get_contents(__DIR__ . '/kartubuku.html');
    ...
    $seq = 0;
    foreach ($_SESSION['kartubuku'] as $id) {
    ...
    // unset($_SESSION['kartubuku']);
    // write to file
    $print_file_name = 'kartubuku_print_result_'.strtolower(str_replace(' ', '_', $_SESSION['uname'])).'.html';
...
<div class="menuBoxInner printIcon">
    ...
    <div class="infoBox">
        ...
        if (isset($_SESSION['kartubuku'])) {
            echo '<strong id="queueCount" class="text-danger">'.@( count($_SESSION['kartubuku']??[])).'</strong>';
